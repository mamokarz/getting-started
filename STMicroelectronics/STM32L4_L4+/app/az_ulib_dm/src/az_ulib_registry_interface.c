...
static az_result registry_1_get_value_concrete(az_ulib_model_in model_in, az_ulib_model_out model_out)
{
  (void)model_out;
  const registry_1_get_model_in* const in = (const registry_1_get_model_in* const)model_in;
  return az_ulib_registry_get_value(in->registry_key, in->registry_value);
}
...
static const az_ulib_capability_descriptor REGISTRY_1_CAPABILITIES[]
    = { AZ_ULIB_DESCRIPTOR_ADD_CAPABILITY(
            REGISTRY_1_GET_VALUE_COMMAND,
            registry_1_get_value_concrete,
            NULL),
        AZ_ULIB_DESCRIPTOR_ADD_CAPABILITY(
            REGISTRY_1_ADD_COMMAND_NAME,
            registry_1_add_concrete,
            NULL) };

static const az_ulib_interface_descriptor REGISTRY_1_DESCRIPTOR
    = AZ_ULIB_DESCRIPTOR_CREATE(REGISTRY_1_INTERFACE_NAME, REGISTRY_1_INTERFACE_VERSION, REGISTRY_1_CAPABILITIES);
